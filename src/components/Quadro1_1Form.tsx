...
export default function Quadro1_1Form() {
  ...
  return (
    <FormContainer 
      title="QUADRO 1.1 NÚMERO DE ESTUDANTES QUE PROCURAM O ENSINO SUPERIOR" 
      description="Preenchimento de dados estatísticos"
      onEnviar={() => alert('Quadro 1.1 enviado para a Repartição de Estatística!')}
    >
        ...
        <table className="w-full border-collapse min-w-max border border-gray-200">
          <thead>
            <tr className="bg-gray-50 text-gray-600 text-[10px] font-black uppercase tracking-widest">
              ...
              <th className="p-4 border border-gray-200" colSpan={3}>ESTUDANTES QUE PROCURAM O ES</th>
            </tr>
            <tr className="bg-gray-50 text-gray-600 text-[10px] font-black uppercase tracking-widest">
          ...
          <tbody>
            {rows.map((row, index) => (
              <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="p-2 border border-gray-200"><input type="number" className="w-16 p-2 border border-gray-200 rounded-lg text-xs" value={row.h} onChange={(e) => {
                    const newRows = [...rows];
                    newRows[index].h = parseInt(e.target.value) || 0;
                    newRows[index].hm = newRows[index].h + newRows[index].m;
                    setRows(newRows);
                }} /></td>
                <td className="p-2 border border-gray-200"><input type="number" className="w-16 p-2 border border-gray-200 rounded-lg text-xs" value={row.m} onChange={(e) => {
                    const newRows = [...rows];
                    newRows[index].m = parseInt(e.target.value) || 0;
                    newRows[index].hm = newRows[index].h + newRows[index].m;
                    setRows(newRows);
                }} /></td>
                <td className="p-2 border border-gray-200 text-xs text-center font-bold">{row.hm}</td>
              </tr>
            ))}
          </tbody>
        </table>
